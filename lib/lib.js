...
function customChecks(element, elementInfo) {
    return !isASentence(getElementByKey(element, elementInfo.getter))
        || myntraCustomCheck(element, elementInfo);
}
...
function isValid(productPrice, element, blacklistedClasses, elementInfo) {
    return !isNaN(productPrice) && productPrice !== 0
        ...
        && customChecks(element, elementInfo);
}
...
function updatePrice(elementInfo, blacklistedClasses) {
    ...
    elements.forEach((element) => {
        ...
        console.log("IS VALID: {}", isValid(productPrice, element, blacklistedClasses, elementInfo))
